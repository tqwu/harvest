...
export class WorkspaceResolver {
  ...
  async countWorkspace(): Promise<WorkspaceCount> {
  ...
  async workspace(@Ctx() request: NextApiRequest): Promise<Workspace[]> {
    const owned = await new WorkspaceService().listOwned(request.user.id);
    const accessible = await new WorkspaceService().listAccessible(
      request.user.id,
    );
